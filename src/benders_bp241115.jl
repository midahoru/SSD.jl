    ...
    if length(rel_to_run) > 0 
        ...
        # for agg in rel_to_run
            while n_iter_rel_clus <= max_rel && keep_relax
                ...
                println("Y2 $(value.(mp[:y]).data)")
                println("red cost $(red_cost.data)")
                y_ind_fixed = findall(>=(50757-lb_temp), red_cost.data) #2824.43 # 50757
                println(y_ind_fixed)
