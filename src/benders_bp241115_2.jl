function model_benders(data, params, status, relaxations=["Clust"], types=["B"], n_outer_cuts=32) #, ρ_h=[], M=[])
    ...
    if solver == "gurobi"
        ...
        Solver_sp_ENV = Gurobi.Env()
    elseif solver == "cplex" 
        Solver_ENV = CPLEX.Env()
        Solver_sp_ENV = CPLEX.Env()
    end
    ...
    for t in T
        if "SH" in types || "PK" in types
            primals[t] = ini_benders_sp_primal(int_y, data, ρ_h, M, t, μ, params, status, solver, Solver_sp_ENV)
        elseif "B" in types || "PK1" in types || "FC" in types
            duals[t] = ini_benders_sp_dual(int_y, data, ρ_h, M, t, params, status, solver, Solver_sp_ENV, types, w_fc, w0)
        elseif "MW" in types
            # duals[t] = ini_benders_sp_dual(int_y, data, ρ_h, M, t, params, status, solver, Solver_sp_ENV, ["B"], w_fc, w0)
            primals[t] = ini_benders_sp_primal(int_y, data, ρ_h, M, t, μ, params, status, solver, Solver_sp_ENV)
            dualsMW[t] = ini_benders_sp_dual(int_y, data, ρ_h, M, t, params, status, solver, Solver_sp_ENV, types, w_fc, w0)
        end
    end
    ...
    if length(rel_to_run) > 0 
        ...
        # for agg in rel_to_run
            while n_iter_rel_clus <= max_rel && keep_relax
                ...
                prim_rel = Dict()
                for t in T
                    prim_rel[t] = ini_benders_sp_primal(int_y, data, ρ_h, M, t, 0, params, status, solver, Solver_sp_ENV, agg)
                end
    ...
    function lazyCB(cb)
        ...
        # if callback_node_status(cb, mp) == MOI.CALLBACK_NODE_STATUS_UNKNOWN # MOI.CALLBACK_NODE_STATUS_INTEGER
        #     return
        # end
        # Test all te nodes. If the y variables are really close to be integer
        ycb = callback_value.(cb, mp[:y])
        yvals = round.(ycb)
        if 1 in (abs.(yvals - ycb) .> 10e-5)
            return
        end
        ...
        #cx_sp_d[i in I, j in J], -π1[j]*λ[i,t] + π2[i] + π3[j]*λ[i,t] - π10[i,j]  <= C[i,j,t]
        #cR_sp_d[j in J], -π7[j] + sum(π8[j,h]*(1-ρ_h[j,t,h])^2 for h in H)  <= 0.5*Dt
        #cρ_sp_d[j in J], -π5[j] + sum(-π8[j,h] for h in H) - π12[j]  <= 0.5*Dt
        #cw_sp_d[j in J, k in K], -π6[j,k] + π7[j] <= 0.5*Dt*cv^2
        #cz_sp_d[j in J, k in K], -π3[j]*Q[j,k] - π4[j,k] + π5[j] - π11[j,k] <= -0.5*Dt*cv^2
    ...
    if "SH" in types || "MW" in types  || "PK" in types
...
function update_sp_primal(primal, data, params, status, solver, y_fixed, t, n_outer_cuts, μ, ρ_h, M, update_ρ=false, agg=[])
    ...
    # set_normalized_rhs([primal[:c1_sp][j] for j in J], reshape(-(1+μ)*sum(Array(data.Q.*y_fixed), dims=2),:))
    
    # set_normalized_rhs.(primal[:c1_sp], -(1+μ)*sum(Array(data.Q.*y_fixed), dims=2))
...
function ini_benders_sp_primal2(y_fixed, data, ρ_h, M, t, μ, params, status, solver, Solver_ENV, agg=[], J_hat, K_hat)
    maxtime = max(1, params.max_time - elapsed(status))
    ...
    # Capacity cannot be exceeded and steady state has to be conserved
    @constraint(m, c1_sp[j in J], -sum(λ[i,t]*x[i,j] for i in I) >= -(1+μ)*Q[j,k] for k in K))
